...
- They mention Kimi / GLM / Qwen frontier / DeepSeek / Grok by name, or vendor host names (Moonshot, Z.ai, Zhipu, QwenCloud, DashScope, Alibaba Cloud, deepseek.com, x.ai), or version strings (K2.6, GLM-5.3, GLM-5.1, Qwen3.8-Max, Qwen3.7-Max, DeepSeek V4, Grok 4.6, Grok 4.5) — load `references/models/kimi.md` / `glm.md` / `qwen-frontier.md` / `deepseek.md` / `grok.md` as relevant
...
GLM options: **GLM-5.3** (current frontier, Aug 2026 — reasoning always on, `reasoning_effort` low/high/max default max, no off-switch) / **GLM-5.2** (previous, Jun 2026) / **GLM-5.1** (Apr 2026) / **GLM-5** (Feb 2026) / **GLM-4.6** (legacy, Sep 2025) / **Universal GLM**.

Frontier Qwen options: **Qwen3.8-Max** (current frontier, GA Aug 2026 — open weights 2.4T-A95B; hosted on QwenCloud (OpenAI- and DashScope-compatible); `reasoning_effort` low/medium/xhigh default xhigh; hosted hybrid / open weights thinking-only) / **Qwen3.7-Max** (previous, May 2026) / **Qwen3.7 Plus** / **Qwen3.6 Plus** / **Qwen3.6 Max-Preview** / **Qwen3-Max-Thinking** (Jan 2026 predecessor) / **Universal frontier Qwen**.

DeepSeek options: **DeepSeek V4-Pro** (current frontier coder/reasoner; April 2026 launch, **GA update 2026-08-13** — effort-model thinking low/high/max, Responses API, peak/off-peak pricing from 16.08) / **V4-Flash** (cost-efficient sibling; official release 31.07) / **V3.2 / V3.2-Speciale** (legacy, Feb 2026) / **R1** (reasoning-only legacy) / **Universal DeepSeek**.
...
> "Which model is this prompt for — Claude (Fable 5 / Opus 5 / Opus 4.8 / 4.7 / Sonnet 5 / Sonnet 4.6 / Haiku 4.5), OpenAI GPT-5.x in Codex CLI (5.6 Sol/Terra/Luna / 5.5 / 5.4 / 5.3), Google Gemini 3.x (3.1 Pro / 3.7 Flash / 3.6 Flash / 3.5 Flash-Lite), Moonshot Kimi (K3 / K2.7-Code / K2.6), Z.ai GLM-5.3 / 5.2 / 5.1 / 5 / 4.6, Alibaba Qwen3.8-Max / 3.7-Max / 3.6, DeepSeek V4-Pro / Flash, xAI Grok 4.6 / 4.5 / 4.3, Mistral frontier, Meta Muse Spark, or cross-vendor?"
...
| Prompt targeting Z.ai GLM-5.3 / 5.2 / 5.1 routed through Claude Code / OpenCode / Cline | (1) On GLM-5.3 / 5.2, set `reasoning_effort` out-of-band as the primary thinking mitigation (`low`/`high`/`max`, default `max` on 5.3; `high`/`max` on 5.2); the `<reasoning_content>` re-injection block is the fallback for routers that don't forward it (and the primary lever on 5.1 and earlier). (2) **On 5.3, `thinking.type: "disabled"` fails the request** — replace any no-reasoning pattern with `enabled` + effort `low`. (3) Strip identity-pinning if present. (4) Trim total load-bearing AGENTS.md size to <4 KiB — 5.3's 1M context does not lift this ceiling (it's a reasoning-gate effect). Cite `models/glm.md § GLM-5.3`, `§ GLM-5.2` and `§ The Claude-Code-router pattern` |
