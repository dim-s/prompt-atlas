...
**DeepSeek side:** system + user prompts for DeepSeek V4-Pro / V4-Flash / V4-Flash-Vision-Exp / V3.2 accessed via DeepSeek's hosted API, self-hosted endpoints, or cross-tool routers. DeepSeek's unusual user-prompt-priority makes vendor-specific overrides (`AGENTS.deepseek.md`) more common than for other vendors. Note the vision model's structural rule: images are accepted in **user messages only** (400 in system/assistant).
...
|---|---|---|
| Claude (Fable 5.1 / Fable 5 / Opus / Sonnet 5 / Haiku) | `effort` (low/medium/high/xhigh/max) | CLI (`/effort xhigh`), API param, **or subagent frontmatter `effort:`** (`.claude/agents/*.md` — official field, overrides session effort) |
...
| xAI Grok 4.6 | `reasoning_effort` (low/medium/high/**xhigh**, default `high`) — **reasoning cannot be disabled** | API param |
...
| DeepSeek V4-Pro / V4-Flash / V4-Flash-Vision-Exp | `thinking` toggle (`enabled`/`disabled` via `extra_body`) + `reasoning_effort` (`low`/`high`/`max`, default enabled with effort `high`); Anthropic surface `reasoning.effort` none/low/high/max; Responses API `output_config.effort` | API param (OpenAI / Anthropic / Responses surfaces). The old `thinking: "off"\|"high"\|"max"` triplet is obsolete since 2026-08-13 |
...
Claude options: **Fable 5.1** (successor to Fable 5, Sep 2026; **Mythos 5.1** = same capabilities, Project Glasswing only) / **Fable 5** (frontier tier above Opus, Jun 2026) / **Opus 5** (current Opus tier, Jul 2026) / **Opus 4.8** / **Opus 4.7** (both now legacy) / **Sonnet 5** (current Sonnet, Jun 2026) / **Sonnet 4.6** (previous) / **Haiku 4.5** / **Universal Claude** / older.

OpenAI options: **GPT-6 Astra** (`gpt-6-astra`, new frontier generation, Sep 2026) / **GPT-5.6** — `sol` / `terra` / `luna` (previous frontier, Jul 2026) / **GPT-5.5** (+ Instant) / **GPT-5.4** / **GPT-5.3 / 5.3-codex** / **GPT-5.2** / **GPT-5.1** (legacy) / **Universal GPT-5.x**.
...
Frontier Qwen options: **Qwen3.8-Max** (current frontier, GA Aug 2026 — open weights 2.4T-A95B; hosted on QwenCloud (OpenAI- and DashScope-compatible); `reasoning_effort` low/medium/xhigh default xhigh; hosted hybrid / open weights thinking-only) / **Qwen3.8-Flash** (cost-efficient sibling, late Aug 2026 — 125B+51B n-gram embedding, 6B active, open weights `Qwen3.8-Flash-Next` (262K native ctx), hosted `qwen3.8-flash` on QwenCloud (1M ctx, built-in tools); thinking knobs unpublished — `?`) / **Qwen3.7-Max** (previous, May 2026) / **Qwen3.7 Plus** / **Qwen3.6 Plus** / **Qwen3.6 Max-Preview** / **Qwen3-Max-Thinking** (Jan 2026 predecessor) / **Universal frontier Qwen**.

DeepSeek options: **DeepSeek V4-Pro** (current frontier coder/reasoner; April 2026 launch, **GA update 2026-08-13** — effort-model thinking low/high/max, Responses API, peak/off-peak pricing from 16.08) / **V4-Flash** (cost-efficient sibling; official release 31.07) / **V4-Flash-Vision-Exp** (experimental multimodal sibling, 21.08 — text on par with V4-Flash, vision-agent near Opus 4.8; images user-messages-only) / **V3.2 / V3.2-Speciale** (legacy, Feb 2026) / **R1** (reasoning-only legacy) / **Universal DeepSeek**.

xAI Grok options: **Grok 4.6** (vendor's recommended model, Aug 2026 — **500K context, down from 4.3's 1M**; `reasoning_effort` low/medium/high/xhigh, default high, cannot be disabled) / **Grok 4.5** (previous recommended, Jul 2026 — 500K, effort low/medium/high) / **Grok 4.3** (previous flagship, 1M context) / Grok 4.20 (predecessor) / Grok 5 (still not shipped as of Aug 2026 — don't tune for it).
...
> "Which model is this prompt for — Claude (Fable 5.1 / Fable 5 / Opus 5 / Opus 4.8 / 4.7 / Sonnet 5 / Sonnet 4.6 / Haiku 4.5), OpenAI GPT-6 Astra / GPT-5.x in Codex CLI (5.6 Sol/Terra/Luna / 5.5 / 5.4 / 5.3), Google Gemini 3.x (3.1 Pro / 3.8 Flash / 3.7 Flash / 3.6 Flash / 3.5 Flash-Lite), Moonshot Kimi (K3 / K2.7-Code / K2.6), Z.ai GLM-5.3-Flash / 5.3 / 5.2 / 5.1 / 5 / 4.6, Alibaba Qwen3.8-Max / 3.8-Flash / 3.7-Max / 3.6, DeepSeek V4-Pro / Flash / Vision-Exp, xAI Grok 4.6 / 4.5 / 4.3, Mistral frontier, Meta Muse Spark 1.3 / 1.2 / Glimmer, Tencent Hy4, or cross-vendor?"
...
2. **Sampling parameters in body** — **Gemini deprecated them API-wide (2026-07-21)**; **newest Claude rejects them** (`temperature`/`top_p`/`top_k` non-default → 400 on Fable 5 / Opus 5 / Opus 4.8 / 4.7 / Sonnet 5; Sonnet 4.6 / Haiku 4.5 still tune); GPT-5.x tunable (GPT-6 Astra: unsupported — remove), Kimi mode-split (1.0 thinking / 0.6 instant, task-shaped top-p on K3), others tunable but vendor handles.
...
5. **"Answer without reasoning" / "don't overthink"** — inert on most vendors, **harmful** on Opus 5 with thinking off (tag leakage), and **structurally unimplementable** where thinking is forced on: Kimi K2.7-Code / K3, **GLM-5.3**, **Grok 4.5 / 4.6**, and the **Qwen3.8 open weights** (which require thinking for all interactions). There is no target where it's the right instruction — strip unconditionally, and on the no-off-switch vendors the only depth lever is the effort parameter.
...
| **Thinking off-switch exists at all** | **Kimi K3 / K2.7-Code, GLM-5.3, Grok 4.5 / 4.6, Qwen3.8 open weights: no** — thinking is forced on (or cannot be disabled); every other vendor exposes some way to reduce or disable it | Never write "answer without reasoning"; if latency is the complaint, it's an effort setting or a model choice |
...
| Prompt targeting xAI Grok 4.6 / 4.5 (or migrated from 4.3) | (1) `[CRITICAL]` if the architecture assumes a 1M window — 4.5 / 4.6 have **500K**; the fix is chunking / retrieval / summarizing carried context, not rewording. (2) Flag accumulated persistent context that can cross the **200K pricing step**, where input and output rates double. (3) If the prompt says "don't think" / "answer immediately", it is **unimplementable** — Grok reasoning cannot be disabled; the lever is the `reasoning_effort` API parameter (default `high`), not prose. No dedicated prompting guide exists for 4.5 / 4.6 — don't invent version-specific wording deltas. Cite `models/grok.md § Grok 4.6` / `§ Grok 4.5` |
