...
| **GPT-5.6 Sol / Terra / Luna** (OpenAI, Jul 9 2026 — current frontier) | high | partial | hurts — strip persona (family rule) | hurts on reasoning | hurts | mostly inert noise | **treats as noise**; **restating an instruction is measured cost** — leaner prompts scored +10–15% at −41–66% tokens |
...
| **Gemini 3.7 Flash** (Aug 13 2026 — current Flash frontier) | high — concise dominates | partial | helps (identity-based, family-consistent) | mixed | mixed | inert | CoT not needed; **`thinking_level` default medium** — set explicitly if depth needed; intro price half of 3.6 Flash |
...
- *"no prompting guide published"* (Grok 4.5, Mistral Medium 3.5, Muse Spark 1.2 / Code / Glimmer, Tencent Hy4 preview) = the vendor documented the model but not how to prompt it. Cells inherit the family default; don't manufacture a version-specific delta that no source supports.
...
| **Gemini 3.7 Flash** (current) | `thinking_level` (low/medium/**default**/high) — default `medium`; set explicitly for depth-critical work | direct, efficient; higher first-pass accuracy (FrontierCode 43.6 vs 34.4 on 3.6) | `response_json_schema` preferred | implicit |
...
| **Meta Model API (Muse Spark 1.2)** | none documented; 1M window | ? — the model **actively manages its own context window**, so harness-side trimming scaffolding is redundant | ? — no persistent-context file convention published |
