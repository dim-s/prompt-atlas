...
| **Grok 4.6** (xAI, Aug 2026 — vendor's recommended model) | medium-high (4.5 family behavior; **no 4.6 prompting guide published**) | yes | OK; no directional preference documented | helps when format strict | helps | inert | tolerated; built-in reasoning — **`reasoning_effort` out-of-band (default high), cannot be disabled** |
...
| **DeepSeek V4-Pro / Flash / Vision-Exp** | **effort model since 2026-08-13** — `thinking` toggle (`enabled`/`disabled`) + `reasoning_effort` low/high/max (default enabled, effort high); Anthropic `reasoning.effort` none/low/high/max; the old `"off"` level is gone. **Tool calls work in thinking mode** (unlike R1). Vision-Exp adds image input (user-messages-only) to the same regime | direct, slightly more literal than Claude | `json_object` + **must still demand JSON in prose** even with `response_format` set | implicit |
...
| **Grok 4.6 / 4.5** (current / previous) | **`reasoning_effort`** — low/medium/high (default high) on 4.5, +xhigh on 4.6; **reasoning cannot be disabled** | direct, terse, action-biased | structured output via OpenAI-compatible API | implicit |
...
**The mirror rule, now with a hard case:** never write "don't reason / answer immediately" either. It's inert prose on every vendor, actively harmful on Opus 5 with thinking disabled (leaks internal tags — `antipatterns.md` #38), and **structurally unimplementable where thinking is forced on**: Kimi K2.7-Code / K3, **GLM-5.3**, **Grok 4.5 / 4.6**, and the **Qwen3.8 open weights**. Latency complaints are an effort setting or a model choice, never a sentence in the prompt.
...
| **Grok 4.5** | tool description preferred | 4.3 family behavior; nothing 4.5-specific documented | yes | yes |
...
| **DeepSeek V4-Pro / Flash / Vision-Exp** | **API breaking from V3.2** on `reasoning_content` (V4 requires round-trip, V3 rejects) | rebuild multi-turn flows to round-trip `reasoning_content`; audit prefix cache hashing; move complex instructions from system → user prompt. Vision-Exp: images user-messages-only + ≤384 tokens/image; migrating V4-Flash→Vision-Exp is text-compatible |
...
| **xAI API (Grok 4.5)** | none documented, but **500K total window** (down from 1M on 4.3) | rot untested; the binding limit is now the window itself | **cost cliff at 200K of prompt** — crossing it doubles input and output rates, so accumulated persistent context has a price boundary, not just a quality one |
