...
| **Meta Muse Spark 1.1** (Jul 9 2026, closed weights, public-preview API) | ? — no prompting guide | ? | ? | ? | ? | ? | ? — but vendor frames work as **goal conditioning**, so outcome-first is the safe default |
...
- *"no prompting guide published"* (Grok 4.5, Mistral Medium 3.5, Muse Spark 1.1) = the vendor documented the model but not how to prompt it. Cells inherit the family default; don't manufacture a version-specific delta that no source supports.
...
| **DeepSeek V4-Pro / Flash** | **effort model since 2026-08-13** — `thinking` toggle (`enabled`/`disabled`) + `reasoning_effort` low/high/max (default enabled, effort high); Anthropic `reasoning.effort` none/low/high/max; the old `"off"` level is gone. **Tool calls work in thinking mode** (unlike R1) | direct, slightly more literal than Claude | `json_object` + **must still demand JSON in prose** even with `response_format` set | implicit |
...
| **Meta Muse Spark 1.1** | ? at the API level — a "Thinking" mode exists in the Meta AI app / meta.ai, no documented API knob | ? | structured output + parallel tool calls (OpenAI-compatible surface) | ? |
...
| **Meta Muse Spark 1.1** | ? — no guidance published; use the cross-vendor default (tool description) | **subagent delegation documented as supported**, along with planning mode, goal conditioning and context compaction; spawn *propensity* undocumented — state what you want | ? | ? |
...
| **Meta Muse Spark 1.1** | from the closed Apr 2026 preview: now a public-preview OpenAI-compatible API | strip harness-side context-management scaffolding (the model self-manages its 1M window); frame work as goals, not steps; no migration path from Llama 4 — Muse Spark is closed-weight, not a Llama successor |
...
| **Meta Model API (Muse Spark 1.1)** | none documented; 1M window | ? — the model **actively manages its own context window**, so harness-side trimming scaffolding is redundant | ? — no persistent-context file convention published |
