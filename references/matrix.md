...
| **Gemini 3.6 Flash** (Jul 21 2026 — current Flash frontier) | high — concise dominates | partial | helps (identity-based, family-consistent) | mixed | mixed | inert | CoT not needed; **output ~17% shorter than 3.5 Flash — carried-over "be brief" overcorrects** |
...
| **Grok 4.5** (xAI, Jul 2026 — vendor's recommended model) | medium-high (4.3 family behavior; **no 4.5 prompting guide published**) | yes | OK; no directional preference documented | helps when format strict | helps | inert | tolerated; built-in reasoning makes scaffolding unnecessary |
...
| **Gemini 3.6 Flash** | `thinking_level` (family enum) | direct, efficient; **~17% fewer output tokens than 3.5 Flash**, fewer unwanted code edits and execution loops | `response_json_schema` preferred | implicit — and terser than its predecessor; don't add a global "be brief" |
...
| **Grok 4.5** | built-in reasoning, **no effort parameter documented** | direct, terse, action-biased (4.3 family) | structured output via OpenAI-compatible API | implicit |
...
**Cross-vendor rule for reasoning depth:** never write "think harder" or "think step by step" in the body. The lever is the API parameter on all current vendors (Claude `effort`, OpenAI `reasoning_effort` — `max` added in 5.6, Gemini `thinking_level`, Kimi `extra_body.thinking` on K2.x / `reasoning effort` on K3, GLM-5.2 `reasoning_effort` / earlier GLM endpoint-default, Qwen mode-toggle, DeepSeek `thinking`). Wording substitutes: "Verify against criteria...", "List edge cases first...".

**The mirror rule, now with a hard case:** never write "don't reason / answer immediately" either. It's inert prose on every vendor, actively harmful on Opus 5 with thinking disabled (leaks internal tags — `antipatterns.md` #38), and **structurally unimplementable on Kimi K2.7-Code and K3**, where thinking is forced on and cannot be switched off at all. Latency complaints are an effort setting or a model choice, never a sentence in the prompt.
