...
**Field-confirmed in agentic / tool-using use (June 2026).** Observed directly on V4-Flash and `deepseek-chat` driving a read-only code-scout via the `llm` CLI (kernel in the `-s` system slot, per-run task via the stdin user turn): with the rules + search methodology sitting in the system prompt, the model **fabricated confident false negatives** on absence questions ("X exists nowhere" / "zero hits" for code that demonstrably existed) and **mis-scoped its searches** (searched the wrong subtree, then concluded). Moving the kernel into the user turn — system kept to a one-line role — fixed both, on the **same model with the same tools** (so it's an instruction-authority effect, not a capability gain). Mechanism: the under-weighted system methodology produced a shallow search that then asserted; in the user turn the methodology lands, the model searches enough to find the real code, and reports reality instead of guessing. Two practical notes: (1) the prompt cache is **unaffected** — the stable kernel stays at the front of the token prefix regardless of role channel, since caching is a prefix match not a per-role thing (measured cache-hit unchanged, ~80–90%); (2) **symptom → diagnosis:** if a DeepSeek agent confidently reports that something *"doesn't exist anywhere"*, suspect system-prompt overuse and verify before trusting the negative.
